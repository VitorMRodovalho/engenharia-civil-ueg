...
if (class_exists('aliroRequest')) $mainframe = mosMainFrame::getInstance();
...
function com_install()
{
	...
	function makeMenuEntry () {
		if (class_exists('aliroRequest')) return;
		$interface =& remositoryInterface::getInstance();
		$database =& $interface->getDB();
	...
	function convertDatabase () {
		...
		if (!in_array('metatype', $fieldnames)) {
			$sql = 'ALTER TABLE `#__downloads_files`'
				.' ADD `metatype` smallint NOT NULL default 0 AFTER keywords;';
			$database->setQuery($sql);
			$database->query();
		}
