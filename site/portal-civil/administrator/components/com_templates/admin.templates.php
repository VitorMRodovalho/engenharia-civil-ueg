...
function removeTemplate( $cid, $option, $client ) {
	...
	if ($cur_template == $cid) {
		mosErrorAlert( "Vocъ nуo pode apagar o tema em uso" );
	}
...
function editTemplateSource( $p_tname, $option, $client ) {
	...
	if ( $fp = fopen( $file, 'r' ) ) {
		...
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operaчуo Falhou: nуo foi possэvel abrir'. $file );
	}
}
...
function saveTemplateSource( $option, $client ) {
	...
	if ( !$template ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operaчуo falhou: Nenhum tema especificado.' );
	}
	if ( !$filecontent ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operaчуo falhou: Conteњdo vazio.' );
	}
	...
	if ( is_writable( $file ) == false ) {
		mosRedirect( 'index2.php?option='. $option , 'Operaчуo falhou: '. $file .' nуo tem permissѕes de escrita.' );
	}
	...
	if ( $fp = fopen ($file, 'w' ) ) {
		...
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operaчуo falhou: Nуo foi possэvel abrir o arquivo para escrita.' );
	}
...
function saveTemplateCSS( $option, $client ) {
	...
	clearstatcache();
	if ( is_writable( $file ) == false ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operaчуo falhou: O arquivo nуo pode ser salvo.' );
	}
