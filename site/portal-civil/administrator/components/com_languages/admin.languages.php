...
function publishLanguage( $p_lname, $option ) {
	...
	if ($fp = fopen('../configuration.php','w')){
		...
		fclose($fp);
		mosRedirect('index2.php?option=com_languages',"Configuraчуo realizada com sucesso! $p_lname");
	} else {
...
function removeLanguage( $cid, $option, $client = 'admin' ) {
	...
	if ($cur_language == $cid) {
		mosErrorAlert( "Vocъ nуo pode apagar idioma em uso." );
	}
...
function editLanguageSource( $p_lname, $option) {
	...
	if ($fp = fopen( $file, "r" )) {
		...
		mosRedirect( "index2.php?option=$option&mosmsg=Operaчуo Falhou: Nуo foi possэvel abrir o arquivo $file" );
	}
}
...
function saveLanguageSource( $option ) {
	...
	if (!$language) {
		mosRedirect( "index2.php?option=$option&mosmsg=Operaчуo Falhou: Nenhum idioma especificado." );
	}
	if (!$filecontent) {
		mosRedirect( "index2.php?option=$option&mosmsg=Operaчуo Falhou: Conteњdo vazio." );
	}
	...
	if (is_writable( $file ) == false) {
		mosRedirect( "index2.php?option=$option&mosmsg=Operaчуo Falhou: O arquivo nуo possui permissуo para escrita." );
	}
	...
	if ($fp = fopen ($file, "w")) {
		...
		mosRedirect( "index2.php?option=$option&mosmsg=Operaчуo Falhou: Nуo foi possэvel abrir o arquivo para escrita." );
	}
}
?>