...
class remositoryInterface {
	...
	function getUser () {
		if (is_callable(array('mamboCore','get'))) $my =& mamboCore::get('currentUser');
		else global $my;
	...
	function prependMetaTag ($tag, $content) {
		$this->getMainFrame();
		$checkmethod = array($this->mainframe, 'prependMetaTag');
		if (is_callable($checkmethod)) $this->mainframe->prependMetaTag($tag, $content);		
	}
