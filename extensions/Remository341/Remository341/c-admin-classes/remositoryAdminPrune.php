...
class remositoryAdminPrune extends remositoryAdminControllers {
	...
	function remositoryAdminPrune ($admin) {
	...
	function listTask (){
		...
		if ($view AND $this->admin->checkCallable($view, 'view')) $view->view($this->ReMOSver, $default_date);
	}
	...
	function saveTask () {
	    ...
	    $default = time() - $d90;
	    if (count($ymd) == 3) {
	        $time = mktime(0,1,0,$ymd[1],$ymd[2],$ymd[0]);
	        $diff = time() - $time;
	        if ($diff < $d90) $time = $default;
	    }
		else $time = $default;
		$startdate = date('Y-m-d H:i:s', $time);
		$sql = "DELETE FROM #__downloads_log WHERE date<'$startdate'";
		remositoryRepository::doSQL($sql);
		echo "<div class='message'>".sprintf(_DOWN_OLD_LOG_REMOVED, $startdate).'</div>';
	}
