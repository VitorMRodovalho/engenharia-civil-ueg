...
class remositoryAddManyFilesHTML extends remositoryUserHTML {
	...
	function fileInputArea ($title, $maxsize, $name, $value, $rows, $cols, $editor) {
		...
		if ($editor) {
			...
			editorArea( 'description', $value, $name, 500, 200, $rows, $cols );
			echo "\n\t\t\t</div>";
		}
		else {
	...
	function addManyFilesHTML($clist)
	{
		...
		$formurl = remositoryRepository::RemositoryBasicFunctionURL('savemanyfiles');
		...
		echo "\n\t\t</dl>";
		$this->simpleTickBox(_DOWN_EXTENSION_IN_TITLE, 'extensiontitle');
		$this->fileInputArea(_DOWN_LICENSE, _DOWN_DESC_MAX, 'license', '', 4, 50, false);
		$this->simpleTickBox(_DOWN_LICENSE_AGREE, 'licenseagree');
